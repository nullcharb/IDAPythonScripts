...
def find_b64_fn_Xrefs(b64_fn_addr, rc4_fn_addr):
    xref_list = []

    for ref in idautils.XrefsTo(b64_fn_addr):
        rc4_call_addr = get_rc4_call_location(ref.frm, rc4_fn_addr)
        xref = {}
        xref['b64'] = ref.frm
        xref['b64hex'] = hex(ref.frm)
        xref['rc4'] = rc4_call_addr
        xref['rc4hex'] = hex(rc4_call_addr)
...
fn_Base64Decode = 0x401806
fn_rc4Decode = 0x408746
not_found_list = []


# 
b64_xref_list = find_b64_fn_Xrefs(fn_Base64Decode, fn_rc4Decode)
# rc4_xref_list, rc4_xref_hex_list = find_b64_fn_Xrefs(fn_rc4Decode)


for item in b64_xref_list:
    b64_string_address = get_fastcall_args_number(item['b64'], 1)
    # b64_string = get_fn_args(item['b64'], 1)
    rc4_key_address = get_stack_args_number(item['rc4'], 3)
    
    print(f"b64fnAddress :{item['b64hex']}, Rc4fnAddress :{item['rc4hex']}")

    print(f"b64StringAddress :{b64_string_address[0]}, keyAddress :{rc4_key_address[0]}")

    
    b64_string = get_value_at_address(b64_string_address[0])
    rc4_key = get_value_at_address(rc4_key_address[0])
    ...
    ret_address = get_return_address(item['rc4'])
    ...
    if len(result) > 0:
        ...
        if len_and_special_char_check(rstring.decode().strip().replace(':', '_')):
            idc.set_name(ret_address, rstring.decode().strip().replace(".","_").replace(':', '_'))
        
        set_comment(item['b64'], rstring.decode())
        idc.set_cmt(ret_address,rstring.decode(),0)
        # idc.set_cmt(ret_address,rstring.decode(),0)
