...
def ida_set_name_and_comment(fn_call, addr, name):
...
for ref in idautils.XrefsTo(fn_addr):
    # print(hex(ref.frm))
    call_addr = get_call_address(ref.frm)
    # print(f"Call addr : {call_addr}")
    if call_addr > 0 :
        fn_list_hex.append(hex(call_addr))
        fn_list.append(call_addr)
...
for fn_call in fn_list:
    ...
    if ret_address > 0:
        ...
        if len(api_name) > 0:
            ...
            ida_set_name_and_comment(fn_call, ret_address, api_name)
        elif 'dw' not in idc.get_name(arg_names[0]):
            # idc.set_name(ret_address, idc.get_name(arg_names[0])+"_0")
            ida_set_name_and_comment(fn_call, ret_address, idc.get_name(arg_names[0]))
        else:
